    ...
    internal class SubRoot_UpdatePowerRating_Patcher
    {
        ...
        public static IEnumerable<CodeInstruction> Transpiler(IEnumerable<CodeInstruction> instructions)
        {
            foreach (var instruction in instructions)
            {
                ...
                {
                    // ((modules.GetCount(TechType.PowerUpgradeModule) <= 0) ? this.currPowerRating : 3f);
                    // Force evalution always false by switching in a TechType that will never be in the Upgrade Console.
                    // This guarantees that we'll always get this.currPowerRating returned which we set in the PreFix.
